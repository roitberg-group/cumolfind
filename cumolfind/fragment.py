...
def cugraph_slice_subgraph_gpu(cgraph, species, nodes):
    ...
    node_expanded = cp.concatenate([cp.full((int(r),), int(
        v), dtype=cp.int32) for v, r in zip(nodes_cp.get(), node_repeats.get())])
    ...
    offsets = cp.arange(len(node_expanded)) - cp.concatenate([cp.full((int(r),), int(
        v), dtype=cp.int32) for v, r in zip((cp.cumsum(node_repeats) - node_repeats).get(), node_repeats.get())])
...
def find_fragments(species, coordinates, cell=None, pbc=None, use_cell_list=True):
    ...
    atom_index12, distances, _ = neighborlist(
        2, species, coordinates, cell=cell, pbc=pbc)
...
def analyze_a_frame(
    ...
    species = cp.array(
        [atom.element.atomic_number for atom in mdtraj_frame.topology.atoms], dtype=cp.int32)
    ...
    mol_database2['flatten_formula'] = mol_database2['flatten_formula'].astype(
        str)
    mol_database2 = cudf.from_pandas(mol_database2)

    df_per_frag["flatten_formula"] = df_per_frag["flatten_formula"].astype(str)

    merged_df_per_frag = mol_database2.merge(
        df_per_frag, on="flatten_formula", how="inner")
    ...
    if len(merged_df_per_frag) == 0:
        ...
        df_formula = df_per_frag["flatten_formula"].value_counts().to_frame(
            "counts").reset_index()
    ...
    global_atom_indices = np.concatenate(
        merged_df_per_frag["atom_indices"].to_pandas().to_numpy())
    ...
    filtered_df = merged_df_per_frag[merged_df_per_frag["fragment_edge_count"]
                                     == merged_df_per_frag["num_edges"]]
    ...
    graph_pandas = graph_pandas.apply(
    ...
    for local_frame, row in filtered_pd.iterrows():
        ...
        fragment_graph = nxgraph.subgraph(frag_atom_indices)
        # pull from preprocessed reference graph
        graph = reference_graphs[local_frame]
        # graphmatcher for cases when number of nodes and edges matches but doesn't correspond to the right molecule
        ...
        if gm.is_isomorphic():
            ...
            match += 1
            print(
                f"    is_isomorphic {row['name']}, flatten_formula {row['flatten_formula']}, match {match}")
