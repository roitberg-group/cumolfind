...
# Combine multiple per-fragment XYZ files into single per-frame XYZ files,
# padding with dummy atoms as needed to ensure consistent atom counts across frames for visualization.
...
def combine_xyz_per_frame(output_dir: Path, tracked_indices: set):
    ...
    if total_count == 0:
        raise RuntimeError(
            "No atoms found in any XYZ files under " + str(output_dir))
