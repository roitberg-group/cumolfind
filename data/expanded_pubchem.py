...
mol_data = pd.DataFrame(
    columns=["graph", "formula", "smiles", "name", "flatten_formula", "category"])
    ...
    while attempt < max_retries:
        try:
            ...
            if not is_CHNO_only(formula):
                warnings.warn(
                    f"Skipping {mol_name} with formula {formula}, contains elements other than CHNO")
                return None
            ...
            with tempfile.NamedTemporaryFile(suffix=".sdf") as temp:
                pubchempy.download(
                    "SDF", temp.name, pubchem_mol.cid, record_type="3d", overwrite=True)
                ase_mol = read(temp.name)

            # Create NetworkX graph
            netx_graph = build_netx_graph_from_ase(
                ase_mol, use_cell_list=False)
            ...
            if "PUGREST.ServerBusy" in str(e):
                ...
                print(
                    f"PubChem server busy (attempt {attempt}/{max_retries}). Retrying in {wait_time} seconds...")
            ...
            if e.code == 503:  # Catch direct HTTP 503 errors
                ...
                print(
                    f"HTTP 503 Error (attempt {attempt}/{max_retries}). Retrying in {wait_time} seconds...")
                time.sleep(wait_time)
            else:
                raise  # Reraise other HTTP errors

    print(
        f"Failed to process {mol_name} after {max_retries} attempts. Skipping...")
    failed_molecules.append(mol_name)


interested_mols = asteroid_data + hcn_polymers + nucleobases + \
    simple_sugars + fatty_acids + amino_acids + dipeptides
