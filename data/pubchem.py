...
mol_data = pd.DataFrame(
    columns=["graph", "formula", "smiles", "name", "flatten_formula"])
...
def process_molecule(mol_name):
    ...
    with tempfile.NamedTemporaryFile(suffix=".sdf") as temp:
        pubchempy.download("SDF", temp.name, pubchem_mol.cid,
...
interested_mols = nucleobases + simple_sugars + \
    fatty_acids + amino_acids + dipeptides
...
mol_data.to_parquet(
    "nucleobases-simple_sugars-fatty_acids-amino_acids-dipeptides.pq")
